...
static lh_value _async_uv_await(lh_resume r, lh_value local, lh_value arg) {
  uv_req_t* req = lh_uv_req_ptr_value(arg);
  req->data = r;
...
uv_stat_t async_stat(const char* path) {
  uv_stat_t stat;
  check_uv_err(async_statx(path, &stat));
  return stat;
}
...
uv_file async_fopen(const char* path, int flags, int mode) {
  uv_file file = -1;
  check_uv_err(async_fopenx(path, flags, mode,&file));
  return file;
}
...
char* async_fread_full(const char* path) {
  char*   contents = NULL;
  ssize_t len = 0;
  check_uv_err(async_fread_fullx(path, &len, &contents));
  return contents;
}
